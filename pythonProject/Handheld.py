...
class HandheldMenu(QWidget):
    def __init__(self):
        ...
        self.setStyleSheet("background-color: #1e1e1e;")    #sets color of background
                                                            #for custom background picture background-color: => background-image:
                                                            #we can modify the picture here(position, repeating, ...)

        # Main layout
        self.layout = QHBoxLayout(self) #sets all self.(QWidget) widgets horizontally and every child of the class QWidget
        ...
        # List of menu items
        self.icons = ['game.png', 'media.png', 'store.png', 'settings.png']  #paths to pictures used as buttons later in the code
        ...
        for i, icon in enumerate(self.icons):
            ...
            # Creates a hover effect on the menu pictures
            btn.setStyleSheet("""
                        border: none;
    ...
    def on_hover_enter(self, button):
        ...
        button.setStyleSheet("background-color: rgba(255, 255, 255, 0.3);")  # Highlights the button
